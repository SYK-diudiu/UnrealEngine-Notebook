...
`Alt+D`：关联性复制，复制出来的物体会随着源物体的改变而改变。

`Shift+S`：用于移动游标到选中项或选中项移动到游标

`Alt+G`：将选中项移动到当前游标处（实际上移动的是选中项自身的原点）

`Ctrl+P`：设置父级目标，用于设置多个物体之间的父子级关系。鼠标第一次选中的物体是儿子，第二个选中的物体是爸爸，之后按下`Ctrl+P`在弹出的窗口中选择**物体**选项即可。
...
`G*2`：选中一个点，按两次`G`，可以让该点沿着点所在线上移动。
...
`Ctrl+A`：应用修改器（需要在物体模式下使用）

`Ctrl+大键盘数字键`：在物体模式下，可以快速给物体添加**数字对应层数**的**表面细分**修改器

## 上色

`U`：通常在编辑模式下，给模型展`UV`。展`UV`之前切记要全选被展物体的点/线/面。
