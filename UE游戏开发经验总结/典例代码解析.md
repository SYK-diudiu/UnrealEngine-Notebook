...
假设存在一个类，它的`XXX`成员变量改变之后会触发`XXXChanged()`函数。

但是我们希望触发`XXXChanged()`之后的代码逻辑在蓝图中执行。

这时可以再写一个函数`OnXXXChanged()`，还需要`UFUNCTION(BlueprintImplementableEvent)`宏标记。

```C++
// 需要实现该函数
virtual void XXXChanged();
...
// 这个函数的实现不需要在C++代码中实现，可以在蓝图中实现该函数
UFUNCTION(BlueprintImplementableEvent)
void OnXXXChanged();
```

运行代码之后打开蓝图中该类的继承对象，之后直接在编辑器中呼出该函数即可。

![image-20250916214906661](典例代码解析.assets/image-20250916214906661.png)

之后执行我们需要执行的蓝图节点即可。

![image-20250916214949658](典例代码解析.assets/image-20250916214949658.png)

这样，当代码中`XXXChanged()`函数执行之后，就会触发蓝图中的打印字符串了。
